...
public class CanvasView {
	...
	public static final String WINDOW_EXCLUIR_TAREFA = "window_excluir_tarefa";
	...
	public static String criaHtmlNovaTarefaWindow() {
		...
		sb.append("		<form class=\"ui-form\" id=\"formWindow\" method=\"post\">                                                                         " + PL);
		...
		sb.append("				<option value=\"0FF\">Amarelo</option>                                                                   " + PL);
		sb.append("			</select>                                                                                                    " + PL);
		...
		sb.append("				<option value=\"1\">Validado</option>                                                                    " + PL);
		sb.append("				<option value=\"-1\">Invalidado</option>                                                                 " + PL);
		sb.append("			</select> <br />                                                                                             " + PL);
		sb.append("			<button id=\"salvarWindow\">Salvar</button>                                                                                             " + PL);
		sb.append("		</form>                                                                                                          " + PL);
		sb.append("	</div>                                                                                                               " + PL);
	...
	public static String criaHtmlExcluirTarefaWindow() {
		StringBuilder sb = new StringBuilder();
		sb.append("	<div id=\""+ WINDOW_EXCLUIR_TAREFA +"\">                                                                                              " + PL);
		sb.append("	Deseja realmente apagar esta tarefa?" + PL);
		sb.append("	</div>" + PL);
	...
	private static Object createBlock(List<TarefaBean> tList, Integer projetoId, int quadro, String title) {
		...
		for (TarefaBean tBean : tList) {
			...
			sb.append("<li class=\"item ui-corner-all\" style=\"background: #"+tBean.getCor()  +"\" tarefaId=\""+ tBean.getId() +"\">");
